                   ...
                   std::string modelWeights, bool bVis)
{
    ...
    if (bVis)
    {
        showDetectedObjects(img, bBoxes, classesFile);
    }
}

void showDetectedObjects(const cv::Mat& img, const std::vector<BoundingBox>& bBoxes, std::string objectClassesFile,
                         const cv::Scalar& bboxFrameColor, string windowName)
{
    // load classes from file
    std::vector<std::string> classes;
    ifstream ifs(objectClassesFile.c_str());
    string line;
    while (getline(ifs, line)) classes.push_back(line);

    // draw bounding boxes enclosing detected objects
    ...
    for (auto it = bBoxes.begin(); it != bBoxes.end(); ++it)
    {
        ...
        cv::rectangle(visImg, cv::Point(left, top), cv::Point(left + width, top + height), bboxFrameColor, 2);

        string label = cv::format("%.2f", (*it).confidence);
        label = "ID:" + std::to_string(it->boxID) + ", " + classes[((*it).classID)] + ":" + label;
