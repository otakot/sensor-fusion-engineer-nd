...
string getDescriptorType(string descriptorExtractor)
{
    return (descriptorExtractor.compare("SIFT") == 0) ? "DES_HOG" : "DES_BINARY";
}
                      ...
                      std::string matcherType, std::string selectorType)
{
    ...
    cout << "   Keypoint matching results for matcher type " << matcherType << " and selector type " << selectorType
         << ":  matched keypoints: " << matches.size() << endl;
}
...
void descKeypoints(vector<cv::KeyPoint> &keypoints, cv::Mat &img, cv::Mat &descriptors, string descriptorType)
{
    ...
    cout << "   " << descriptorType << " descriptor extraction results: descriptors: " << descriptors.size()
...
void detKeypointsHarris(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, bool bVis)
{
    ...
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    cout << "   Harris keypoints detection results: " << keypoints.size() << " keypoints. Duration: " << 1000 * t / 1.0
         << " ms" << endl;
...
void detKeypointsModern(std::vector<cv::KeyPoint> &keypoints, cv::Mat &img, std::string detectorType, bool bVis)
{
    ...
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    cout << "   " << detectorType.c_str() << " detection results: " << keypoints.size()
