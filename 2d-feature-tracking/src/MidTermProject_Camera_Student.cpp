...
int main(int argc, const char *argv[])
{
    ...
    // supprted keypoint detectors: SHITOMASI, HARRIS, FAST, BRISK, ORB, AKAZE, SIFT
    vector<string> detectors = {"SHITOMASI", "HARRIS", "FAST", "BRISK", "ORB", "AKAZE", "SIFT"};
    // supported keypoint descriptors: BRISK, BRIEF, ORB, FREAK, AKAZE, SIFT
    vector<string> descriptors = {"BRISK", "BRIEF", "ORB", "FREAK", "AKAZE", "SIFT"};
    // supported matcher type values: MAT_BF, MAT_FLANN
    string matcherType = "MAT_BF";
    // supported match selector type values: SEL_NN, SEL_KNN
    ...
    for (auto detectorName : detectors)
    {
        for (auto descriptorName : descriptors)
        {
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
            {
                ...
                {
                    detKeypointsModern(keypoints, imgGray, detectorName);
                }
                ...
                if (bFocusOnVehicle)
                {
                    ...
                    if (bVis)
                    {
                        visualizeKeypoints(keypoints, imgGray, detectorName);
                    }
                    cout << "   Keypoints in preceding vehicle ROI: " << keypoints.size() << endl;
                }
                ...
                cv::Mat descriptors;
                descKeypoints(dataBuffer.rbegin()->keypoints, dataBuffer.rbegin()->cameraImg, descriptors,
                              descriptorName);
                ...
                if (dataBuffer.size() > 1)  // wait until at least two images have been processed
                {
                    ...
                    string descriptorType = getDescriptorType(detectorName);
                    cout << "   matchDescriptors: Detector: " << detectorName << ", Descriptor: " << descriptorName
                         << ", Descriptor type: " << descriptorType << ", matcherType: " << matcherType
                         << ", matcherType: " << selectorType << endl;
