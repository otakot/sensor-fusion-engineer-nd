                  ...
                  const pcl::PointCloud<pcl::PointXYZI>::Ptr& input_cloud)
{
    ...
    // FILTER POINT CLOUD
    //const auto& filteredCloud = pointProcessorI->FilterCloud(inputCloud, 0.15f , Eigen::Vector4f (-10.f, -6.3f, -2.2f, 1), Eigen::Vector4f ( 30.f, 6.3f, 2.f, 1));
    const auto& filtered_cloud = point_processor->filterCloud(input_cloud, kFilterResolution,
        Eigen::Vector4f(kFilterBBoxMinX, kFilterBBoxMinY, kFilterBBoxMinZ, 1),
        Eigen::Vector4f(kFilterBBoxMaxX, kFilterBBoxMaxY, kFilterBBoxMaxZ, 1));

    // CLOUD SEGMENTATION
    const auto& segmented_clouds = point_processor->segmentPlane(
        filtered_cloud, kSegmentationMaxIterations, kSegmentationDistanceThreshold);

    // PCL CLUSTERING
    const auto& cloud_clusters = point_processor->clustering(
        segmented_clouds.first, kClusteringDistanceThreshold, kClusteringMinSize, kClusteringMaxSize);
...
int main (int argc, char** argv)
{
    ...
    // RenderCityBlock(viewer, pointProcessorI, inputCloud);
