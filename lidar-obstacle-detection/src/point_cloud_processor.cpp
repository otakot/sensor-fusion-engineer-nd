...
std::vector<int> findClusterPoints(int cluster_seed_point_idx, PointCloudPtr<PointT> const &cloud,
                                   KdTree<PointT> *const &tree, std::vector<bool> &processed_point_ids,
                                   float distanceTol)
{
    ...
    for (const auto &nearby_point_idx : nearby_point_ids)
    {
        ...
        {
            const auto transitive_cluster_point_ids =
                findClusterPoints(nearby_point_idx, cloud, tree, processed_point_ids, distanceTol);
            std::move(transitive_cluster_point_ids.begin(), transitive_cluster_point_ids.end(),
                      std::back_inserter(cluster_of_point_ids));
        }
    }
...
template <typename PointT>
std::vector<std::vector<int>> euclideanClustering(PointCloudPtr<PointT> const &cloud, KdTree<PointT> *const &tree,
                                                  float distanceTol, uint16_t min_size, uint16_t max_size)
{
    ...
    for (int point_idx = 0; point_idx < cloud->points.size(); point_idx++)
    {
        ...
        {
            std::vector<int> cluster_of_points =
                findClusterPoints(point_idx, cloud, tree, processed_point_ids, distanceTol);
...
template <typename PointT>
std::vector<PointCloudPtr<PointT>> PointCloudProcessor<PointT>::clustering(PointCloudPtr<PointT> cloud,
                                                                           float cluster_tolerance, uint16_t min_size,
                                                                           uint16_t max_size)
{
    ...
    std::cout << "Clustering took " << elapsed_time.count() << " milliseconds and found " << clusters.size()
              << " clusters" << std::endl;
