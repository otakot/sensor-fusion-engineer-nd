...
struct Node
{
    ...
    Node(PointT point, int setId) : point(point), id(setId), left(NULL), right(NULL) {}
...
struct KdTree
{
    ...
    std::pair<float, float> getCompareCoordinatesForLevel(size_t level, const PointT& target_point,
                                                          const PointT& tree_root_point) const
    {
    ...
    void addPointAsTreeNode(Node<PointT>*& tree_root, std::size_t level, PointT point, int id)
    {
        ...
        else
        {
            ...
            std::tie(point_coord, tree_root_point_coord) =
                getCompareCoordinatesForLevel(level, point, tree_root->point);
    ...
    void insert(PointT point, int id) { addPointAsTreeNode(this->root, 0 /* root level */, point, id); }

    bool isPointInsideBoundaryBox(const PointT& point, const PointT& box_center_point,
                                  float box_boundary_dist_from_center) const
    {
    ...
    void findNearbyPointsInTree(Node<PointT>* const& tree_root, std::size_t tree_level, const PointT& target_point,
                                float distanceTol, std::vector<int>& result) const
    {
        if (tree_root != NULL)
        {
            ...
            // now continue search of nearby tree points in each of split areas of current tree point which overlap
            // theboundary box of target point
            float target_point_coord, tree_root_point_coord;
            std::tie(target_point_coord, tree_root_point_coord) =
                getCompareCoordinatesForLevel(tree_level, target_point, tree_root->point);
